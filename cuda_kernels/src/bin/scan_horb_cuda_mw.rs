...
fn score_candidate(
    ...
    let cells = build_mass_cells(n_side, extent_kpc, a0, dm_mass)?;
...
fn main() -> Result<()> {
    ...
    if args.len() != 8 {
        bail!(
            "usage: scan_horb_cuda_mw <rc_csv> <baryons_csv> <out_csv> <r_min> <r_max> <n_side> <extent_kpc>"
        );
    }
    ...
    let a0_list = env::var("HORB_A0_LIST").unwrap_or_else(|_| "0.8,0.9,1.0,1.1,1.2,1.3".to_string());
    let mass_list = env::var("HORB_DM_MASS_LIST")
        .unwrap_or_else(|_| "1.5e11,2e11,2.5e11,3e11,3.5e11".to_string());
    let softening_list = env::var("HORB_SOFTENING_LIST").unwrap_or_else(|_| "0.25".to_string());
    ...
    for a0 in a0_values {
        for dm_mass in mass_values.iter().copied() {
            for softening in softening_values.iter().copied() {
                eprintln!(
                    "scoring 3d_z2 a0={:.3} mass={:.6e} n_side={} extent={} softening={}",
                    a0, dm_mass, n_side, extent_kpc, softening
                );
